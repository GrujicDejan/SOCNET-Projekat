...
public class SlashdotTest {
	...
	public static <V, E> void main(String[] args) {
		...
		try (Scanner sc = new Scanner(System.in)) {
			UndirectedSparseGraph<Node, SignedLink> slashdotGraph = ImportRealNetwork.readSlashdotOrEpinions(FILE, MAX_LINES);
			ComponentClustererBFS<Node, SignedLink> ccbfs = new ComponentClustererBFS<Node, SignedLink>(slashdotGraph, signTransformer);
			
			Menu<V, E> menu = new Menu<>(signTransformer, "slashdot");
			menu.printMenu();
			System.out.println("\n-------------------------------\nVas izbor: ");
			int in = sc.nextInt();
			while (in != 0) {
