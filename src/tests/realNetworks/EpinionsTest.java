...
public class EpinionsTest {
	
	public static String FILE = "res/soc-sign-epinions.txt";
	...
	public static <V, E> void main(String[] args) {
		...
		try (Scanner sc = new Scanner(System.in)) {
			UndirectedSparseGraph<Node, SignedLink> epinionsGraph = ImportRealNetwork.readSlashdotOrEpinions(FILE, MAX_LINES);
			ComponentClustererBFS<Node, SignedLink> ccbfs = new ComponentClustererBFS<Node, SignedLink>(epinionsGraph, signTransformer);
			
			Menu<V, E> menu = new Menu<>(signTransformer, "epinionsGraph");
			menu.printMenu();
			System.out.println("\n-------------------------------\nVas izbor: ");
			int in = sc.nextInt();
			while (in != 0) {
